...
export default function Home() {
  ...
  // LOGIKA SALDO ADA DI SINI (Sudah Benar)
  const stats = useMemo(() => {
    ...
    return { 
      income, 
      expense, 
      total: income - expense // Ini adalah Saldo Wallet yang benar
    };
  }, [transactions]);

  const handleAddTransaction = async (tx: Transaction) => { 
    await addTransaction(tx); 
    await loadData(); // Reload data akan memicu useMemo update saldo otomatis
  };
  ...
  return (
    <main className="flex flex-col h-full w-full bg-background relative overflow-hidden">
      ...
      {/* 1. HEADER & BUDGET CARD */}
      {activeTab === 'home' && (
        <div className="shrink-0 z-20 bg-background w-full">
          <Header />
          {/* PERBAIKAN: Kirimkan stats.total sebagai balance */}
          <BudgetCard 
            balance={stats.total} 
